...
class StatsController extends Controller
{
    public function indexAction()
    {
        ...
        $query = $em->createQuery("SELECT t.villeId, count(t) total FROM BackOfficeBundle:Trajet t group by t.villeId order by total desc limit 5");

        $resultat = $query->getResult();
        $top_depart = $resultat;

        $query = $em->createQuery("SELECT t.villeId1, count(t) total FROM BackOfficeBundle:Trajet t group by t.villeId1 order by total desc limit 5");
