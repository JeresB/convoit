...
class StatsController extends Controller
{
    public function indexAction()
    {
        ...
        $query = $em->createQuery("SELECT IDENTITY(t.villeId) as ville_id, v.ville as ville, count(t) total FROM BackOfficeBundle:Trajet t, BackOfficeBundle:Ville v group by t.villeId order by total desc")->setMaxResults(5);

        $resultat = $query->getResult();
        $top_depart = $resultat;

        $query = $em->createQuery("SELECT IDENTITY(t.villeId1) as ville_id, v.ville as ville, count(t) total FROM BackOfficeBundle:Trajet t, BackOfficeBundle:Ville v group by t.villeId1 order by total desc")->setMaxResults(5);
